    ...
    def insert(self, base, rdn, attrs=None):
        ...
        for attr_key, attr_val in attrs.items():
            if attr_key.endswith(';binary'):
                is_binary = True
                attr_key = attr_key[:-7]
            elif attr_key.endswith(';integer'):
                is_integer = True
                attr_key = attr_key[:-8]
            else:
                is_binary = False
            ...
            if attr_val != ['']:
                if not is_binary:
                    attr_val = list(map(encodeToUtf8Bytes, attr_val))
    ...
    def modify(self, dn, mod_type=None, attrs=None):
        ...
        for key, values in attrs.items():
            ...
            if key.endswith(';binary'):
                key = key[:-7]
            elif key.endswith(';integer'):
                key = key[:-8]
                values = list(map(int, values))
            else:
                values = list(map(encodeToUtf8Bytes, values))

            if mod_type is None:
                if cur_rec.get(key, [b'']) != values and values != [b'']:
